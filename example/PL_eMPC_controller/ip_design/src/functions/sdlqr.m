function [k,s,e,ad,bd,Qd,Rd,Nd] = sdlqr(a,b,q,r,nn,Ts)
%SDLQR  Sampled-data linear-quadratic regulator design
%
%   [K,S,E,Ad,Bd,Qd,Rd,Nd] = SDLQR(A,B,Q,R,Ts)  calculates the optimal gain matrix K 
...
%
%   [K,S,E,Ad,Bd,Qd,Rd,Nd] = SDLQR(A,B,Q,R,N,Ts) handles the more general cost function
%   J = Integral {x'Qx + u'Ru + 2*x'Nu} dt.
%
%   (Qd,Rd,Nd) are the discrete weighting matrices. Nd is non-zero, in
%   general, even if N=0.
%
...
%   using DLQR.
%
%   See also  LQRD, DLQR, LQR, C2D, and SS/KALMD.

%   Clay M. Thompson 7-16-90
%   Revised: P. Gahinet  7-25-96
%   Copyright 1986-2005 The MathWorks, Inc.
%   $Revision: 1.13.4.2 $  $Date: 2005/12/22 17:37:37 $
%   Modified by E.C. Kerrigan 12-8-2010
...
% removed the following three lines because not necessary for FFOH
% formulation
% if min(vr)<=0,
%    error('The R matrix must be positive definite.')
% else
if min(vqnr)<-1e2*eps*max(0,max(vqnr)),
