...
int write_stimuli(double input_data[], unsigned packet_internal_ID)
{
	...
	{
	case 0: //x0_in
		// store x0_in into ../../ip_design/test/stimuli/lqr_controller/x0_in.dat
		pFile = fopen ("../../ip_design/test/stimuli/lqr_controller/x0_in.dat","w+");

		for (i = 0; i < X0_IN_LENGTH; i++)
		{
			fprintf(pFile,"%2.18f \n",input_data[i]);
		}
		fprintf(pFile,"\n");
	...
	case 1: //x_ref_in
		// store x_ref_in into ../../ip_design/test/stimuli/lqr_controller/x_ref_in.dat
		pFile = fopen ("../../ip_design/test/stimuli/lqr_controller/x_ref_in.dat","w+");

		for (i = 0; i < X_REF_IN_LENGTH; i++)
		{
			fprintf(pFile,"%2.18f \n",input_data[i]);
		}
		fprintf(pFile,"\n");
...
{
	system("vivado_hls -f ../../.metadata/lqr_controller_ip_design_test.tcl");
}
...
int read_results(double output_data[], unsigned packet_internal_ID, unsigned packet_output_size)
{
	...
	{
	case 0: //u_in
		// load ../../ip_design/test/results/lqr_controller/u_out.dat
		pFile = fopen ("../../ip_design/test/results/lqr_controller/u_out.dat","r");
